...
export async function loadDashboard() {
  ...
  const todaySales = sales.filter(s => (s.date?s.date.startsWith:undefined)(td));
  const monthSales = sales.filter(s => (s.date?s.date.startsWith:undefined)(thisMonth));
  ...
  sales.forEach(s => {
    const d = (s.date?s.date.split:undefined)("T")[0];
