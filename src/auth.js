...
async function _renderLoginUsers() {
  ...
  if (!active.length) {
    list.innerHTML = '<div style="text-align:center;color:#a1a1aa;font-size:13px;padding:20px">Nenhum utilizador encontrado.<br/>Faz reset da app.</div>';
    return;
  }
  list.innerHTML = active.map(function(u) {
    ...
    var label   = isAdmin ? "Administrador" : "Operador de Caixa";
    return '<button onclick="window._selectUser(' + u.id + ')" style="display:flex;align-items:center;gap:14px;padding:14px 16px;background:' + bg + ';border:2px solid ' + border + ';border-radius:14px;cursor:pointer;font-family:inherit;text-align:left;width:100%">' +
      '<div style="width:48px;height:48px;background:' + color + ';border-radius:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:20px;font-weight:700;color:#fff">' + (u.avatar || u.name.charAt(0).toUpperCase()) + '</div>' +
      '<div><div style="font-size:16px;font-weight:700;color:#18181b">' + u.name + '</div><div style="font-size:12px;color:' + color + ';margin-top:2px">' + label + '</div></div>' +
      '</button>';
  }).join("");
}
...
window._selectUser = async function(userId) {
  ...
  if (badge) {
    ...
    badge.innerHTML = '<div style="width:40px;height:40px;background:' + color + ';border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff">' + (_selectedUser.avatar || _selectedUser.name.charAt(0).toUpperCase()) + '</div><div><div style="font-size:15px;font-weight:700;color:#18181b">' + _selectedUser.name + '</div><div style="font-size:12px;color:' + color + '">' + (isAdmin ? "Administrador" : "Operador de Caixa") + '</div></div>';
  }
...
function _initPinKeypad() {
  window._pinKey = function(key) {
    ...
    _pinBuffer += key;
    _updatePinDots();
    if (_pinBuffer.length === 6) setTimeout(_tryLogin, 150);
  };
}
...
function _shakePin() {
  ...
  setTimeout(function() { _pinBuffer = ""; _updatePinDots(); }, 500);
}
...
async function _tryLogin() {
  ...
  if (openSession) {
    ...
    var sessionId = await db.add("sessions", {
      uuid, userId: _selectedUser.id, userName: _selectedUser.name,
      status: "open", openedAt: new Date().toISOString(), closedAt: null,
      prevSessionUuid: lastClosed ? (lastClosed.uuid || null) : null,
      stockRecebido, stockEsperado: {}, totalVendas: 0, nVendas: 0,
      hasIncidents: false, validated: false, validatedBy: null, validatedAt: null,
...
export function logout() {
  ...
  overlay.style.cssText = "position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px";
  overlay.innerHTML =
    ...
    "<svg width='28' height='28' viewBox='0 0 24 24' fill='none' stroke='#dc2626' stroke-width='2'><path d='M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4'/><polyline points='16 17 21 12 16 7'/><line x1='21' y1='12' x2='9' y2='12'/></svg></div>" +
    "<div style='font-size:18px;font-weight:700;margin-bottom:8px'>Terminar sessão?</div>" +
    "<div style='font-size:13px;color:#71717a;margin-bottom:20px;line-height:1.5'>Fecha o turno antes de sair para não perder dados.</div>" +
    "<div style='display:flex;gap:10px'>" +
    "<button id='logout-cancel' style='flex:1;padding:13px;background:#f4f4f5;color:#71717a;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit'>Cancelar</button>" +
    "<button id='logout-confirm' style='flex:1;padding:13px;background:#dc2626;color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit'>Sair</button>" +
    "</div></div>";
  document.body.appendChild(overlay);
  document.getElementById("logout-cancel").onclick  = function() { overlay.remove(); };
  document.getElementById("logout-confirm").onclick = function() { overlay.remove(); _doLogout(); };
}
...
export async function createUser(name, username, password, role) {
  ...
  return db.add("users", {
    name, username, passwordHash, password: null, role, active: true,
